plot_model_interaction <- function (P.anal, m, eff2plot, modvar2plot, plot_points=TRUE, plot_residuals=FALSE, export_plot=TRUE, ylabel = NULL, fontname = "Almoni ML v5 AAA",
  ...
  #       tmp_effplot <- interact_plot(model = m, pred = year_ct, modx = subunit, data = P.anal)
  ...
  if (eff2plot %in% c("settlements","agriculture","dunes","habitat","land_use","subunit")) {
    ...
    #inland sands
    #predlabels <- as.character(as.numeric(c("2017","2018", "2019","2020", "2021")))
  } 
  ...
  modx_title <- switch(modvar2plot,
                       ...
                       "dunes" = "סיווג הדיונה",
                       "habitat" = "בית הגידול",
                       "land_use" = "",                                         # remove legend title because of lack of space
                       "year_ct" = "שנה",
                       "subunit" = "תת-יחידה")
  ...
  if (modvar2plot %in% c("year_ct","Distance_rescaled","Distance_agri_rescaled")){
    modxvals = 'plus-minus'
  } else if (modvar2plot%in% c("agriculture","settlements","subunit","land_use","habitat","dunes")) {
    modxvals = NULL
  }
  ...
  eval(parse(text=eval(expr = paste0("effplot <- interact_plot(model = m, data=P.anal, pred = ",
                                     ...
                                     ", modx.values = modxvals, modx.labels = modlabs,
                                     line.colors = 'black', point.alpha = 0.25, x.label = xlabel, 
                                     y.label = ylabel)"))))
  
  if (eff2plot=="year_ct") {
    effplot <- effplot + scale_x_continuous(breaks=c(0,2,4,6,8), labels = predlabels)
    #inland sands
    #effplot <- effplot + scale_x_continuous(breaks=c(5,6,7,8,9), labels = predlabels)
